...
for i = 1:length(tamanhos)
    ...
    hashCodes = zeros(1,length(keys)); % cada palavra tem os seus hash codes
    % count time
    tic
    for j = 1:length(keys) % percorrer palavras
        % h(k) = k mod M
        % k -> key
        % M -> tamanho da tabela
        hashCodes(j) = mod(string2hash(keys{j}),length(vector)) + 1;
...
for i = 1:length(tamanhos)
    ...
    for j = 1:length(keys) % percorrer palavras
        ...
        % k -> key
        % M -> tamanho da tabela
        hashCodes(j) = mod(string2hash(keys{j},'sdbm'),length(vector)) + 1;
        if vector(hashCodes(j)) > 0 % Se a posicao do hashcode obtido foi alterada
            %antes, há colisao
            collision = collision + 1;
        end
...
for i = 1:length(tamanhos)
    ...
    for j = 1:length(keys) % percorrer palavras
        ...
        % k -> key
        % M -> tamanho da tabela
        hashCodes(j) = mod(hashstring(keys{j},length(vector)),length(vector)) + 1;
        if vector(hashCodes(j)) > 0 % Se a posicao do hashcode obtido foi alterada
            %antes, há colisao
            collision = collision + 1;
        end
...
for i = 1:length(tamanhos)
    ...
    for j = 1:length(keys) % percorrer palavras
        ...
        % k -> key
        % M -> tamanho da tabela
        hashCodes(j) = mod(DJB31MA(keys{j},127),length(vector)) + 1;
        if vector(hashCodes(j)) > 0 % Se a posicao do hashcode obtido foi alterada
            %antes, há colisao
            collision = collision + 1;
        end
